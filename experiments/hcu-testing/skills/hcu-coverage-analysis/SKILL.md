...
Format user-facing conclusions according to [the standard output format](references/output-format.md). When the user asks for results in chat, use its concise structure rather than returning only attachments or raw JSON. For multi-branch audits, report each branch independently before comparing them.
